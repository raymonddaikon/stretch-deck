<script lang="ts">
	...
	type Props = {
		...
		tiltRange?: number;
		class?: string;
		/** Optional view-transition-name for the deck container */
		viewTransitionName?: string;
		/** Optional function to get view-transition-name for a card. Receives card and index. */
	...
	function handleClick(event: MouseEvent) {
		...
		if (isClickOverCard(frontCardIndex, event.clientX, event.clientY)) {
			// Stop propagation to prevent parent handlers (like navigation) from firing
	...
	const shareUrl = $derived(`${page.url.origin}/add?type=deck&id=${deck?.$jazz.id}`);
</script>
		...
		{#snippet renderCard(index)}
			...
			{#if card?.$isLoaded}
				<Card
					...
					{card}
					{aligned}
					viewTransitionName={getViewTransitionName?.(card, index)}
				/>
			{/if}
		{/snippet}
	</Snap>

	{#if deck && showCompleteButton}
		<div class="complete-button-wrapper">
			<button class="button" disabled={!allCardsCompleted} onclick={() => onDeckComplete?.()}>
				{m.complete()}
			</button>
		</div>
	{/if}
...
<style>
	...
	/* Disable expensive filter during view transitions for smoother animation */
	:global(html:has(::view-transition)) .deck-container {
		filter: none;
	}
